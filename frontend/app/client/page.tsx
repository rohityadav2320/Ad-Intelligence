      ...
      {playerAd?.video_url && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-6" onClick={() => setPlayer(null)}>
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            <video src={`${API}${playerAd.video_url}`} className="max-h-[80vh] max-w-[90vw] rounded-lg" controls autoPlay />
          </div>
        </div>
      )}
              ...
              <div className="flex gap-4 mb-5">
                {detailAd.video_url && (
                  <div className="w-40 h-28 bg-gray-100 rounded-xl flex-shrink-0 overflow-hidden relative cursor-pointer group"
                    ...
                    <video src={`${API}${detailAd.video_url}`} className="w-full h-full object-cover"
                      muted
                      ...
                      <div className="shrink-0">
                        {reel.video_url ? (
                          <div className="w-24 h-16 bg-gray-100 rounded-xl overflow-hidden relative cursor-pointer group"
                            ...
                            <video src={`${API}${reel.video_url}`} className="w-full h-full object-cover" muted
                              onMouseEnter={e => (e.currentTarget as HTMLVideoElement).play()}
                              onMouseLeave={e => { const v = e.currentTarget as HTMLVideoElement; v.pause(); v.currentTime = 0; }}
                            />
                            <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        >
                          ...
                          <td className="px-4 py-3 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                            <div className="flex gap-2 items-center">
                              ...
                              {ad.video_url && <a href={`${API}/api/download?path=${encodeURIComponent(ad.video_url)}`} className="text-gray-400 hover:text-blue-600" title="Download"><Download className="w-4 h-4" /></a>}
                              <button
                                onClick={async () => {
